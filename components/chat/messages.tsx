...
function AILogo() {
  return (
    <div className="w-9 h-9">
      <Image src="/leonardo dicapital-modified.png" alt={AI_NAME} width={120} height={120} />
    </div>
  );
}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="px-3 py-1 bg-[hsl(var(--prompt-bg))] rounded-2xl text-white max-w-[60%] shadow-sm hover:shadow-md transition-shadow duration-300"
      >
        {message.content}
      </motion.div>
    </motion.div>
  ...
  return (
    ...
    >
      ...
      <motion.div
        whileHover={{ scale: 1.01 }}
        transition={{ type: "spring", stiffness: 300, damping: 20 }}
        style={{ color: "hsl(250, 22%, 18%)" }}
        className="px-3 py-1 bg-[hsl(var(--response-bg))] rounded-2xl max-w-[60%] shadow-sm hover:shadow-md transition-shadow duration-300"
      >
...
function EmptyMessages() {
  return (
    <div className="flex flex-col flex-1 p-1 gap-3 justify-center items-center">
      <p className="text-[hsl(var(--foreground))]">Ask a question to start the conversation</p>
    </div>
  );
}
